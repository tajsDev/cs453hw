...
{
  //write code here
  // note DIM is unused parameter - REMOVE IT LATER
  int POI = threadIdx.x + ( blockDim.x * blockIdx.x ); // point of interest -- corresponds to 1 row in distanceMatrix
  unsigned int COL;
  // pseudocode:
  // for each cell in distMat, if cell < e, inc count for p_0
  // ie, each thread gets a row (corresponds to p_i)

  if ( POI < N ) {
    resultSet[ POI ] = 0;
    for ( COL = 0; COL < N; COL++ ) {
      if ( distanceMatrix[ POI * N + COL ] < epsilon ) {
        resultSet[ POI ] += 1;
      }
    }
  }
